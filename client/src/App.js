...
function App() {
  ...
  const generatePrediction = async () => {
    ...
    setLoading(true);
    try {
      const imageData = images.map((image) => image.base64_file);
      const data = { image: imageData };
      const apiUrl = process.env.REACT_APP_API_URL || 'http://127.0.0.1:5000/';
      const res = await axios.post(apiUrl, data, { timeout: 30000 });
      setPredictedImage(images);
      setPredictions({ image: imageData, result: res?.data?.result ?? [] });
      setShowPrediction(true);
    } catch (err) {
      console.error('Prediction request failed:', err);
      alert('Prediction failed. Please check the backend server or network connection.');
    } finally {
      setLoading(false);
    }
  }
